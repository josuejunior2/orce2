    ...
    @section('content')
        <div class="card m-3">
            <div class="card-header">
                <h3 class="card-title">Cadastro de Site l2l no orçamento {{ $orcamento->titulo }}</h3>
            </div>
        ...
        @if ($orcamento->sitesOrcamento->isNotEmpty())
            <div class="card m-3" style="height: 14rem">
                <div class="card-body card-body-scrollable card-body-scrollable-shadow">
                    @foreach ($orcamento->sitesOrcamento as $key => $site)
                        <div class="card mb-2">
                            <div class="card-header">
                                <h3 class="card-title">{{ $site->nome }}</h3>
                            </div>
                            <div class="card-body">
                                <div class="datagrid">
                                    <div class="datagrid-item">
                                    ...
                                    <div class="datagrid-item">
                                        ...
                                        <div class="datagrid-content">{{ $site->Cidade->nome }}</div>
                                    </div>
                                    <div class="datagrid-item">
        ...
        <div class="card m-3">
            <div class="card-body">
                <form method="POST" action="{{ route('site.store.l2l') }}" enctype="multipart/form-data" onsubmit="adicionarTexto()">
                    @csrf
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md">
                            <div class="mb-3">
                                @if($isPontaA)
                                    <label class="col-form-label required">Nome da Ponta A</label>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="row g-2">
                                    <div class="col">
                                        ...
                                        <input type="text" class="form-control" id="coord" value="{{ old('coord', '') }}" autocomplete="off">
                        ...
                        <div class="col-md">
                            <div class="mb-3">
                                <label class="col-form-label required">Cidade</label>
                                <div class="col">
                                    <select class="form-select" type="text" name="cidade_id" id="cidade_id">
                                        <option value="">-- Selecione uma cidade --</option>
                                        @foreach ($cidades as $c)
                                            <option value="{{ $c->id }}"
                                                {{ old('cidade_id') == $c->id ? 'selected' : '' }}>
                                                {{ $c->nome }} - {{ $c->Estado->uf }}
                                            </option>
                                        @endforeach
                                    </select>
                ...
                <div class="col-sm-3">
                    ...
                    <div class="card-footer text-end">
                        <button type="submit" class="btn btn-primary">
                            ...
                            @if(isset($botao)) {{ $botao }} @elseif($isPontaA) Cadastrar Ponta A @else Cadastrar site @endif
                        </button>
                    </div>
                </form>
            </div>
...
@section('js')
    ...
    <script>
        function adicionarTexto() {
            const isPontaA = "{{ $isPontaA }}";
            const input = document.getElementById("nome");
            const textoAntes = "(Ponta A) ";

            if (isPontaA && !input.value.startsWith(textoAntes)) {
                input.value = textoAntes + input.value;
            }
        }
    </script>
@endsection
